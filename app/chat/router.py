...
from app.chat.services import detail_group, my_chats, send_messages
...
async def api_my_chat_groups(current_user: Users = Depends(get_current_user)):
    group_in_admin = await ChatDAO.my_group_chat(id_created=current_user.id)
    group_in_user = await ChatDAO.invited_my_group_chat(id_user=current_user.id)
    return group_in_admin + group_in_user
